...
// Enhanced upload photo with database storage (create OR update)
exports.uploadPhoto = async (req, res) => {
  try {
    ...
    // ✅ Check if photo already exists (based on filename or originalName)
    ...
    if (existingPhoto) {
      // ✅ Photo already exists → just add user to uploadedBy
      savedPhoto = await Photo.findByIdAndUpdate(
        existingPhoto._id,
        { $addToSet: { uploadedBy: userObjectId } }, // prevents duplicates
        { new: true }
      ).populate("uploadedBy");

      console.log("Uploader added to existing photo:", savedPhoto);
    } else {
      // ✅ Create new photo
      const photo = new Photo({
        ...
        uploadedBy: [userObjectId], // start array with uploader
      });

      savedPhoto = await photo.save();
      console.log("New photo created:", savedPhoto);
    }

    res.json({
      status: true,
      message: existingPhoto
        ? "Uploader added to existing photo"
...
exports.deletePhoto = async (req, res) => {
  try {
    ...
    if (photo.uploadedBy.length === 0) {
      // If no one is left, delete the whole photo
